    ...
    class ReduxFramework_extension_custom_fonts {
        ...
        function processWebfont($attachment_id, $name, $mime_type, $subfolder = 'custom/') {
            ...
            if (!is_dir($this->upload_dir)) {
                //$this->filesystem->mkdir($this->upload_dir, FS_CHMOD_DIR);
                $this->parent->filesystem->execute('mkdir', $this->upload_dir);
            }
            if (!is_dir($this->upload_dir . $subfolder )) {
                //$this->filesystem->mkdir($this->upload_dir . $subfolder, FS_CHMOD_DIR);
        ...
        private function getMissingFiles($name, $fontname, $missing, $output, $subfolder) {
            ...
            if (!is_dir($this->upload_dir . 'missing')) {
                //$this->filesystem->mkdir($this->upload_dir . 'missing', FS_CHMOD_DIR);
                $this->parent->filesystem->execute('mkdir', $this->upload_dir . 'missing');
            }
            ...
            foreach ($missing as $item) {
                ...
                $this->parent->filesystem->execute('put_contents', $this->upload_dir . 'missing/' . $name . '.' . $item . ".tar.gz", $param_array);
                
                //$this->filesystem->put_contents($this->upload_dir . 'missing/' . $name . '.' . $item . ".tar.gz", $response->body, FS_CHMOD_FILE);
                ...
                try {
                    ...
                    try {
                        ...
                        $this->filesystem->delete($this->upload_dir . 'missing/', true, 'd');
            ...
            if (!empty($output)) {
                foreach ($output as $key => $value) {
                    ...
                    $this->parent->filesystem->execute('copy', $value, $param_array);
                    //$this->filesystem->copy($value, $this->upload_dir . $subfolder . $name . '/' . $fontname . '.' . $key, FS_CHMOD_DIR);
                }
            }
