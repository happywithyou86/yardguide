...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
        ...
        class TGM_Plugin_Activation {
                 ...
                 * @var string
                 */
                public $domain = 'swift-framework-admin';
                ...
                public function __construct() {
                        ...
                        $this->strings = array(
                                'page_title'                      => __( 'Install Required Plugins', 'swift-framework-admin' ),
                                'menu_title'                      => __( 'Install Plugins', 'swift-framework-admin' ),
                                'installing'                      => __( 'Installing Plugin: %s', 'swift-framework-admin' ),
                                'oops'                            => __( 'Something went wrong.', 'swift-framework-admin' ),
                                ...
                                'return'                          => __( 'Return to Required Plugins Installer', 'swift-framework-admin' ),
                                'plugin_activated'                => __( 'Plugin activated successfully.', 'swift-framework-admin' ),
                                'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'swift-framework-admin' ),
                        );
                ...
                public function install_plugins_page() {
                        ...
                        <div class="tgmpa wrap">
                                ...
                                <?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), 'swift-framework-admin' ); ?>
 								
								<div class="sf-plugin-note-wrap clearfix">
									<h3>Please Read!</h3>
									<p>The plugins below are plugins that we either provide as premium plugins that are bundled with the theme, or plugins that we recommend in order to take advantage of theme functionality.</p>
									<p>In all cases if you don't need a certain plugin, or aren't sure, then please do not install/activate them - until you decide they are needed. Installing more plugins than you need will likely slow your site down. For example, unless you really need to, then we'd recommend only installing one or two (maximum) of the slider plugins.</p>
									<p>Once you've installed a plugin, you'll need to also click the "Activate" link, and then reload the browser to see the plugin's menu.</p>
								</div>
                        ...
                        if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
                                ...
                                if ( $this->is_automatic ) {
                                        ...
                                        if ( is_wp_error( $activate ) ) {
                                                ...
                                                echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'swift-framework-admin' ) . '</a></p>';
                                ...
                                foreach ( $this->plugins as $plugin ) {
                                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                                echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( $this->strings['return'], 'swift-framework-admin' ) . '</a></p>';
                                                $complete[] = $plugin;
                                                break;
                                        }
                                        /** Nothing to store */
                                ...
                                if ( empty( $complete ) ) {
                                        echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'swift-framework-admin' ) . '">' . __( 'Return to the Dashboard', 'swift-framework-admin' ) . '</a>' ) . '</p>';
                        ...
                        elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
                                ...
                                if ( is_wp_error( $activate ) ) {
                                        ...
                                        if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
                                                $msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'swift-framework-admin' ), '<strong>' . $plugin['name'] . '</strong>' );
                        ...
                        if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
                                ...
                                if ( ! empty( $message ) ) {
                                        ...
                                        foreach ( $message as $type => $plugin_groups ) {
                                                ...
                                                $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'swift-framework-admin' ), $imploded, $count ) . '</p>'; // All messages now stored
                                        }
 
                                        /** Setup variables to determine if action links are needed */
                                        $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'swift-framework-admin' ) . '</a>' : '';
                                        $show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'swift-framework-admin' ) . '</a>'  : '';
                                        ...
                                        $action_links = apply_filters(
                                                ...
                                                array(
                                                        ...
                                                        'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'swift-framework-admin' ) . '</a>',
                                                )
                                        );
