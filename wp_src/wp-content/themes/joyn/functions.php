<?php
	...
	*/
	...
	new WPUpdatesThemeUpdater_964( 'http://wp-updates.com/api/2/theme', basename( get_template_directory() ) );
	...
	add_filter(  'gettext',  'change_portfolio_to_events'  );
add_filter(  'ngettext',  'change_portfolio_to_events'  );

function change_portfolio_to_events( $translated ) {
  $translated = str_ireplace(  'Post',  'Event',  $translated );  // ireplace is PHP5 only
  return $translated;
}

add_filter(  'gettext',  'change_projects_to_events'  );
add_filter(  'ngettext',  'change_projects_to_events'  );

function change_projects_to_events( $translated ) {
  $translated = str_ireplace(  'Projects',  'Events',  $translated );  // ireplace is PHP5 only
  return $translated;
}
